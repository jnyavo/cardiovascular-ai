...
from model.IMLE_net.configs.imle_config import Config
...
def build_imle_net(config: Config, sub=False) -> tf.keras.Model:
    ...
    attention_dim = getattr(config, 'attention_dim', 64)
...
def build_model(weight_name = 'imle_net_community_weights', weight_path = "ckpts") -> tuple[tf.keras.Model, Config]:
    """ Build the model and load the pretrained weights.

    Parameters
    ----------
    name: str
        The name of the weight. (default: 'imle_net_weights')
    weight_path: str
        The path to the weights. (default: "checkpoints")
    Returns
    -------
    tf.keras.Model
        The keras sequential model.
    ...
    config = Config()
    path_weights = os.path.join(os.getcwd(), weight_path, f"{weight_name}.h5")
